...
namespace scythe {
	...
	void Mesh::CreatePhysicalBox(float size_x, float size_y, float size_z, float size_u, float size_v, const Vector3 * position)
	{
		...
		// Set bounds
		Vector3 max = Vector3(size_x, size_y, size_z);
		Vector3 min = -max;
		if (!has_bounds_)
		{
